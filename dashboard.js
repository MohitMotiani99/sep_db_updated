...
MongoClient.connect(url,function(err,db){
        ...
        if(err) throw err
          ...
          new Promise((resolve,reject)=>{
            data.Tags.forEach(word => {
              console.log(word)
              JSON.parse(body).filter((question) => {return question.Tags.indexOf(word)>-1}).map((question) => {console.log('Hi');q_set.add(JSON.stringify(question))})
            })
            resolve()
          }).then(()=>{
            console.log(q_set)
    ...
    app.post('/searchcusts',(req,res)=>{
        ...
        dbo.collection(collection2).find({}).toArray((err,result)=>{
          res.send(result.filter((u)=>{return u.username.indexOf(search_name)>=0}))
        })
    })
    ...
    //     await dbo.collection(collection2).find({}).toArray(async (err,result)=>{
    //       if(err) throw err

    //       await result.filter((user)=>{return user.username.indexOf(searchname)>=0}).map((user)=>{
    //         var sol={
    //           'user':user
    //         }
    //         new Promise((resolve,reject)=>{
    ...
    //           request.get({
    //             headers:{'content-type':'application/json'},
    //             url:`http://localhost:5050/users/${user.Id}/totalquestions`
    //           },(err,response)=>{
    //               if(err) throw err
    //               console.log(response.body)
    //               sol['q_count']=Number(response.body)
    //               resolve()
    //           })
